...
if (!function_exists('treevar')) {
  function treevar (&$var, $open_all = false, $check_dublicates = false, $limit_vars = -1) {
    $GLOBALS['treevar_check_dublicates'] = $check_dublicates;
    $GLOBALS['i_treevar_vars'] = 0;
    $GLOBALS['l_treevar_vars'] = $limit_vars;
    $GLOBALS['treevar_objects'] = [];
    if (empty($GLOBALS['l_treevar_vars'])) $GLOBALS['l_treevar_vars'] = -1;
    $open_first_level = true;
    $result = '
    <style>
      .treevar {
            display: block;
    ...
    $result .= '<div class="treevar'.($open_first_level?'':' dClose').($open_all?' dOpenAll':'').'"><!--
                --><div class="dParent'.($open_first_level?'':' dClose').'"><!--
                  --><div class="dInline"><div class="dInlineBlock dSquare dBtnOpenAll">'.($open_all?'c':'o').'</div>&nbsp;TreeVar&nbsp;<div class="dInlineBlock dSquare dBtnOpen">'.($open_first_level?'-':'+').'</div></div><!--
                  --><div class="dChilds '.($open_first_level?'':'dNone').'">';
    //$result .= '<div class="treevar'.($open_all?'':' dClose').'"><!-- -->';

    $openVar = function ($k, &$v, $openVar, $open_all, &$chain){

      if ($GLOBALS['treevar_check_dublicates']) {
        foreach ($GLOBALS['treevar_objects'] as &$ob) {
          if ($ob === $v) {
            $str_val = (is_array($v) ? 'Array' : get_class($v));
            return '<div class="dVdash"></div><div class="dInline">[<span class="dc_red2">' . $k . '</span>]&nbsp;:&nbsp;<span class="dc_red2">' . (gettype($v)) . '</span>&nbsp;=&gt;&nbsp;<span class="dc_red2">' . $str_val . '</span></div><br />';
          }
        }
        unset ($ob);
      }
      if (is_array($v) || is_object($v)) {
        if (empty($chain)) {
          ...
          $chain_inner = ['p' => &$chain,'e' => $v];
        }
      }

      if ($GLOBALS['l_treevar_vars'] !== -1) {
        if ($GLOBALS['l_treevar_vars'] < $GLOBALS['i_treevar_vars'])
          return '';
        ++$GLOBALS['i_treevar_vars'];
      }
      ...
      $getChain = function ($key, &$value, $openVar, $open_all, &$chain) {
        $chain_inner2 = ['p' => &$chain['p'], 'e' => &$chain['e']];
        return $openVar($key, $value, $openVar, $open_all, $chain_inner2);;
      };
      foreach ($v as $key => &$value) {
        //$chain_inner2 = ['p' => $chain_inner['p'], 'e' => $chain_inner['e']];
        //$s .= $openVar($key, $value, $openVar, $open_all, $chain_inner);
        $s .= $getChain($key, $value, $openVar, $open_all, $chain_inner);
      }
      $s .= '</div></div>';
      //$s .= '</div>';
      unset ($value);
    ...
    $chain_outer = ['p' => null,'e' => []];
    $chain_outer = [];
    $result .= $openVar('',$var, $openVar, $open_all, $chain_outer);
            ...
            var attach_listeners = function (tree_var) {
                ...
                var addListener_open_all = function (b) {
                    b.addEventListener("click", function (e) {
                        ...
                        if (!tree_var.classList.contains("dOpenAll")) {
                          ...
                          tree_var.classList.add("dOpenAll");
                          b.innerHTML = "c";
                        } else {
                          i = 0, l = parents.length;
                          while (i < l) {
                          ...
                          tree_var.classList.remove("dOpenAll");
                          b.innerHTML = "o";
                        }
                    });
                };
